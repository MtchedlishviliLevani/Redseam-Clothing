...
import NoProductsFound from "@/features/listing/NoProductFound";
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <p className="text-[12px] text-[#3e424a]">
              Showing {product?.meta?.from ?? 0}-{product?.meta?.to ?? 0} of{" "}
              {product?.meta.total}
            </p>
                  ...
                  <div key={product.id}>
                    <Link href={`/listing/${product.id}`}>
                      <Image
                        ...
                        width={100}
                        height={100}
                        className="w-full rounded-[10px] shadow-md shadow-gray-400 object-cover"
                      />
                      <div className="mt-3 space-y-[2px]">
                        <h2 className="text-[#10151f] text-[18px] font-medium capitalize">
                          {product.name}
                        </h2>
                        <p className="text-[#10151f] text-[16px] font-medium">
                          $ {product.price}
                        </p>
                      </div>
                    </Link>
